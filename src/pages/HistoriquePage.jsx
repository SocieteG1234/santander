...
export default function HistoriquePage({ navigate }) {
  ...
  const filtered = (currentUser?.transactions || []).filter((op) => {
    const matchSearch = op.libelle
      .toLowerCase()
      .includes(search.toLowerCase());

    const matchFiltre =
      filtre === "all" || op.type === filtre;
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-3 sticky top-0 z-50">
        <div className="max-w-lg mx-auto flex items-center gap-4">
          ...
          >
            <ArrowLeft size={22} className="text-[#0B1F3A]" />
          </button>
      ...
      <main className="max-w-lg mx-auto w-full px-4 py-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4">
            ...
            <p className="text-xl font-bold text-green-700">
              +{formatMontant(totalCredit)} {currentUser?.devise}
            </p>
          </div>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
            ...
            <p className="text-xl font-bold text-orange-700">
              -{formatMontant(totalDebit)} {currentUser?.devise}
            </p>
          </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#C9A227] text-sm"
          />
        ...
        <div className="flex gap-2">
          ...
          {[["all", "Tout"], ["credit", "Crédits"], ["debit", "Débits"]].map(
            ([val, label]) => (
              <button
                key={val}
